<?php 
	class Sales_offlineModelsCat extends FSModels
	{
...
//			$limit   = $current_parameters->getParams('limit');
		...
		function getCategory()
		{
			...
			$query = " SELECT *
						FROM ".$fs_table -> getTable('fs_sales_offline_categories')." 
...
//            var_dump($code);
            global $db;
            $query = " SELECT *
					FROM fs_sales_offline_categories 
					WHERE 
						published = 1 AND level = 1 AND alias_wrapper LIKE '%".$code."%'
