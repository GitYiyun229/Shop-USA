...
//				$this -> _update($row3, 'fs_products_images',' `id` = '.$item->id);
		...
		function  get_main_image_for_news($image ){
			$arr_img_paths = array (array ('resized', 120,120, 'resized_not_crop'),array('small',80,80,'resized_not_crop'));
...
//			FSRemote:: save_image('http://vidic.com.vn/media/news/'.$image.'.jpg',1,'/media/news/',0);
		
//			$folder_image_begin = 'D:\xampp\htdocs\svn\vidic\code\media\news\\';
			$folder_image_begin = 'D:\xampp\htdocs\msmobile\code\images\news\upload_img\\'.$filename;
			$day = '24';
			$month = '11';
			$folder_image_destination = 'D:\xampp\htdocs\msmobile\code\images\news\2014\\'.$month.'\\'.$day.'\original\\';
			$fsFile = FSFactory::getClass('FsFiles','');
			...
			return 'images/news/2014/'.$month.'/'.$day.'/original/'.$filename;
//				$this -> _update($row3, 'fs_products_images',' `id` = '.$item->id);
		}
