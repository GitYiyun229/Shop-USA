...
//								array("1","h&#227;ng s&#7843;n xu&#7845;t"),
            ...
            $this -> table_name = FSTable_ad::_('fs_'.$this -> type.'_filters');
		...
		function getFieldList($type){
			...
			$query = " SELECT * FROM $this->fs_tables
					WHERE type = '$type'
					AND field_name <> 'id' 
					AND is_filter = 1";
		...
		function save($row = array(), $use_mysql_real_escape_string = 1){
			...
			// field
			$field_current = $this  -> get_record(' type = "'.$this -> type.'" AND field_name = "'.$field_name.'"',$this->fs_tables);
			// foreign:
			if(isset($field_current) && !empty($field_current) && ($field_current ->field_type == 'foreign_one' || $field_current ->field_type == 'foreign_multi')){
				...
				if(!$this -> save_filter_new($field_name,$field_show, $field_current))
					return false;
					
				if( !$this -> save_filter_edit($field_name, $field_current))
					return false;
			}
			// calculate filter:
		...
		function save_filter_new($field_name,$field_show, $field_current){
			global $db;
			...
			if($total){
				...
				for($i = 0 ; $i < $total ; $i ++ )	{
					...
					if( $filter_show && $alias ){
						...
						//$type = 'fs_'.$this -> type;
                        $type = FSTable_ad::_('fs_'.$this -> type);
						$arr_insert[]  = 		"('$filter_show','$alias','$type','$field_name','$field_show','$calculator','$value','$field_ordering',$field_ordering_item,'$published',1)";
					}
				}
				// insert into table fs_tables
				if(count($arr_insert))	{
					$sql_insert .= " INSERT INTO ".$this -> table_name." ";
					$sql_insert .= "		(filter_show,alias,tablename,field_name,field_show,calculator,filter_value,field_ordering,field_ordering_item,published,is_common) ";
					$sql_insert .= "		 VALUES "; 
		 ...
		 * Change field exist
		 */
		function save_filter_edit($field_name, $field_current){
			global $db;
			$fsstring = FSFactory::getClass('FSString','','../');	
			// EXIST FIELD
			$total = FSInput::get('filter_exist_total');
			...
			for($i= 0 ; $i < $total ; $i++)	{
				...
				if($filter_show_exist){
					...
					if( ($filter_show_exist != $filter_show_exist_begin) || ($calculator_exist != $calculator_exist_begin) || ($value_exist != $value_exist_begin) || ($published_exist != $published_exist_begin) || ($alias_exist != $alias_exist_begin)){
			...
			foreach($foreign as $item){
				if(in_array($item -> id, $arr_foreign_id_select)){
                    ...
                    if(!$check_exist){
						...
						$row['tablename'] = FSTable_ad::_('fs_'.$this -> type);
